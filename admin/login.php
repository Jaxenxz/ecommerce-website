...
if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
    // Check if the user is not an admin, then display a warning message and log them out
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate credentials and retrieve user data from the database
    if (empty($email_err) && empty($password_err)) {
        // Prepare a select statement
        $sql = "SELECT id, email, password, user_group FROM users WHERE email = ?";
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->num_rows == 1) {
                    ...
                    $stmt->bind_result($id, $email, $hashed_password, $user_group);
                    if ($stmt->fetch()) {
                        // Verify the password
                        if (password_verify($password, $hashed_password)) {
                            ...
                            // Check if the user is an admin, if not, logout
                            if ($user_group !== "admin") {
                                session_destroy(); // Destroy all session data
